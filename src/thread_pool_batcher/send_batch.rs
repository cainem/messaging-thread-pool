    ...
    P: PoolItem,
{
    ...
    pub fn send_batch<'a, const N: usize, V>(&'a self) -> Vec<V>
    where
        V: From<ThreadRequestResponse<P>> + RequestResponseMessage<N, false>,
        //&'a V: Into<ThreadRequestResponse<P>>,
    {
        todo!();
        // self.thread_pool()
        //     .upgrade()
        //     .expect("threads to live for the life of the process")
        //     .send_and_receive(self.to_send().borrow().as_slice())
        //     .collect()
    }
}
