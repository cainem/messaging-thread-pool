...
impl From<SumResponse> for ThreadRequestResponse<Randoms> {
    fn from(response: SumResponse) -> Self {
        ThreadRequestResponse::MessagePoolItem(RandomsApi::Sum(RequestResponse2::Response(
            response,
        )))
    }
}
...
impl From<ThreadRequestResponse<Randoms>> for SumResponse {
    fn from(response: ThreadRequestResponse<Randoms>) -> Self {
        let ThreadRequestResponse::MessagePoolItem(RandomsApi::Sum(RequestResponse2::Response(response))) = response else {
            panic!("unexpected")
        };
        response
    }
